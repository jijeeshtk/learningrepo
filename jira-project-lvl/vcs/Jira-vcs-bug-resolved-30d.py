...
DEFAULT_JQL = f'project = VCS AND type IN (Bug, Defect) AND resolutiondate >= -{LOOKBACK_HOURS}d'
...
def main():
    log(f"Running Jira Bug/Defect resolved report for last {LOOKBACK_HOURS}d…")
    log(f"JQL: {JQL}")
    ...
    if total == 0:
        msg = f"No Bugs/Defects resolved in the last {LOOKBACK_HOURS}d. (Heartbeat)"
        if TEAMS_MODE == "card":
            post_to_teams_card("Bug Resolved Report — No Updates", [])
        else:
    ...
    for idx, b in enumerate(batches, start=1):
        header = f"Bug Resolved Report — Batch {idx}/{total_batches} (items {(idx-1)*BATCH_SIZE + 1}..{(idx-1)*BATCH_SIZE + len(b)})"
