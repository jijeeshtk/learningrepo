...
FIELDS = [
    ...
    "customfield_11034","customfield_10001","customfield_11067","customfield_11062","customfield_11055","updated"
]
...
def format_issue(issue: Dict) -> Dict:
    ...
    if isinstance(sdata, list): sprint_value = ", ".join(nz(s.get("name")) for s in sdata if isinstance(s, dict))
    elif isinstance(sdata, dict): sprint_value = nz(sdata.get("name"))
    ...
    summary = nz(f.get("summary"));  summary = summary if len(summary) <= 180 else summary[:177] + "..."
    return {
        "key": nz(issue.get("key")), "summary": summary,
        "IssueType": nz((f.get("issuetype") or {}).get("name")),
        "Status": nz((f.get("status") or {}).get("name")),
        "Priority": nz((f.get("priority") or {}).get("name")),
        "Assignee": assignee_value, "Reporter": reporter_value,
        "EpicLink": nz(f.get("customfield_10014")), "Created": safe_date(f.get("created")),
        "Updated": safe_date(f.get("updated")), "Resolved": safe_date(f.get("resolutiondate")),
        "Sprint": sprint_value, "AffectsVersions": affects_versions, "FixVersions": fix_versions,
        "Customers": customers_value, "ScrumTeams": nz((f.get("customfield_11034") or {}).get("value")),
        "Teams": nz((f.get("customfield_10001") or {}).get("name")),
        "RootCause": nz((f.get("customfield_11067") or {}).get("value")),
        "BugMaturity": to_string(f.get("customfield_11062")), "ReleasePackage": nz(f.get("customfield_11055")),
...
def post_to_teams(text: str):
    if DRY_RUN: print(f"[DRY_RUN] Would post to Teams:\n{text[:8000]}\n---"); return
    if not TEAMS_WEBHOOK_URL: print("ERROR: TEAMS_WEBHOOK_URL is not set.", file=sys.stderr); sys.exit(4)
    resp = requests.post(TEAMS_WEBHOOK_URL, headers={"Content-Type":"application/json"}, data=json.dumps({"text": text}), timeout=30)
    if resp.status_code < 200 or resp.status_code >= 300:
        print(f"ERROR: Teams webhook returned {resp.status_code}: {resp.text}", file=sys.stderr); sys.exit(6)
...
def main():
    ...
    with open("jira.json","w",encoding="utf-8") as f: json.dump({"issues": formatted}, f, indent=2, ensure_ascii=False)
    total = len(formatted); print(f"Total issues: {total}")
    if total == 0:
        post_to_teams(f"No Bugs/Defects updated in the last {LOOKBACK_HOURS}h. (Heartbeat)"); print("Posted heartbeat to Teams."); return
    batches = chunk(formatted, BATCH_SIZE)
    for idx, b in enumerate(batches, start=1):
        header = f"Bug Updated Report — Batch {idx}/{len(batches)} (items {(idx-1)*BATCH_SIZE+1}..{(idx-1)*BATCH_SIZE+len(b)})"
        body = json.dumps({"issues": b}, indent=2, ensure_ascii=False)
        text = f"{header}\n```json\n{body}\n```"
        if len(text.encode("utf-8")) > 25000:
            lines = [f"- {i['key']} | {i['Status']} | {i['Priority']} | {i['Assignee']} | {i['summary']}" for i in b]
            text = f"{header}\n" + "\n".join(lines)
        post_to_teams(text); print(f"Posted batch {idx}/{len(batches)} to Teams."); time.sleep(1)
...
if __name__ == "__main__":
    try: main()
        ...
        print(f"ERROR: Unhandled exception: {e}", file=sys.stderr); sys.exit(9)
