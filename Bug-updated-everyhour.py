...
def format_issue(issue: Dict) -> Dict:
    ...
    if len(summary) > 180: summary = summary[:177] + "..."
...
def post_to_teams(text: str):
    ...
    if not TEAMS_WEBHOOK_URL:
        print("ERROR: TEAMS_WEBHOOK_URL is not set.", file=sys.stderr); sys.exit(4)
    resp = requests.post(TEAMS_WEBHOOK_URL, headers={"Content-Type": "application/json"},
                         data=json.dumps({"text": text}), timeout=30)
...
def main():
    ...
    if total == 0:
        ...
        print("Posted heartbeat to Teams."); return
